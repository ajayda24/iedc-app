...
export function dateChip(iso: string): { mon: string; day: string } {
  ...
  return {
    mon: d
      .toLocaleString('en-US', { month: 'short', timeZone: APP_TIMEZONE })
      .toUpperCase(),
    day: d.toLocaleString('en-US', { day: '2-digit', timeZone: APP_TIMEZONE }),
  }
}
...
export function timeRange(startIso: string, endIso: string | null): string {
  ...
  // Compare calendar days in the app timezone, not the viewer's, so the
  // same-day collapse matches the times we actually render.
